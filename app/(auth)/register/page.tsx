...
export default async function RegisterPage(props: { searchParams: Promise<{ message: string }> }) {
  ...
  const signUpWithGoogle = async () => {
    ...
    if (error) {
      redirect('/register?message=Erreur+authentification+Google')
    }
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-zinc-900 py-8 px-4 border border-zinc-800 shadow sm:rounded-2xl sm:px-10">
          ...
          <form action={signUpWithGoogle}>
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-zinc-700 rounded-xl shadow-sm text-sm font-medium text-white bg-zinc-800 hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 focus:ring-offset-zinc-900 transition-colors cursor-pointer"
            >
          ...
          {searchParams?.message && (
            <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm font-medium text-center">
              {searchParams.message}
            </div>
          )}
